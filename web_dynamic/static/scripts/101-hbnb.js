  ...
  $.ajax({
    ...
    success: function (places, status) {
      ...
      // $.each(places, function (i, place) {
        var listRev = [];
        $.getJSON('http://0.0.0.0:5001/api/v1/places/' + places[i].id + '/reviews', function (resp) {
          for (let i = 0; i < resp.length; i++) {
            listRev.push(resp[i].text);
          }
          // console.log(listRev);
          $('.places').append(
            $('<article>')
              .append(
                ...
                $('<div>').addClass('review')
                  .append(
                    $('<h3>').text('Reviews ').append(
                      $('<span>').text('show').addClass('show_rev'))
                  ),
                ('<div class="showBut hidden">' + listRev.join(', ') + '</div>')
              )
          );
          $('.show_rev').click(function () {
            if ($('.showBut').hasClass('hidden')) {
              $('.showBut').removeClass('hidden').addClass('showed');
            } else {
              $('.showBut').removeClass('showed').addClass('hidden');
            }
          });
        });
      }
